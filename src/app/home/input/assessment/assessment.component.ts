...
export class AssessmentComponent implements OnInit {
  ...
  {
    let x = document.getElementById("assessment-"+String(this.stepper._steps.length-1));
    x.innerHTML="Save Changes";
    if(i >= this.stepper._steps.length-1)
    {
      this.final_button.nativeElement.disabled = false;
      (document.getElementById("assessment-"+String(this.stepper._steps.length-1)) as HTMLButtonElement).disabled=true;
    }
  }
  ...
  PO_submit(num)
  {
    ...
    this.co[num].CO_Assessment.push(this.PO_details.controls['po_pso'].value);
    this.show = true;
